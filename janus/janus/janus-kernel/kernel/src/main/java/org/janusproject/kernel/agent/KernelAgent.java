...
import org.janusproject.kernel.message.MigrationRequestMessage;
import org.janusproject.kernel.message.MigrationStatusMessage;
import org.janusproject.kernel.message.MigrationStatusMessage.MigrationStatus;
...
extends ActivatorAgent<AgentActivator> {
	...
	/** This Map contains agentAddress of agents which are
	 * currently waiting for a migration to another kernel.
	 * The long parameters is timestamp max at which we consider
	 * migration failed. It's a timeout
	 */
	private final Map<AgentAddress, Long> migrationWaitingAgentAddress = new HashMap<AgentAddress, Long>();


	/**
	 * This map contains agents which have been migrated to this kernel, but we wait
	 * the confirmation from the other kernel before we resuscitate them.
	 */
	private final Map<AgentAddress, Agent> migrationWaitingResuscitate = new HashMap<AgentAddress, Agent>();

	/**
	 * Represents the timeout, in milliseconds, at which a migration 
	 * without response must be canceled
	 * Units : milliseconds 
	 */
	private final long TIMEOUT_MIGRATION_WAITING = 10000;



	/**
	 * True if the migrationTask is already running
	 * Used to not launch two instance of task.
	 */
	private boolean migrationTaskLaunched = false;
	...
	/**
	 * Check and process migrationsMessage waiting in Mailbox
	 * Note : must be called before clearMailbox in processPrivateBehavior
	 */
	private void checkMigrations() {
		Mailbox m = getMailbox();

		for (Message message : m) {
			if(message instanceof MigrationRequestMessage) {
				processMigrationRequestMessage( (MigrationRequestMessage) (message));
			}
			else if(message instanceof MigrationStatusMessage) {
				processMigrationStatusMessage( (MigrationStatusMessage) (message));
			}
		}
	...
	/**
	 * Process a MigrationRequestMessage
	 * @param migrationRequestMessage the message to process
	 */
	private void processMigrationRequestMessage(
			MigrationRequestMessage migrationRequestMessage) {

		AgentAddress sender = migrationRequestMessage.getSender();
		AgentAddress agentMigratingAddress = migrationRequestMessage.getAgentMigratingAddress();
		try {
			//get the agent in the message & deserialize it
			Agent agentMigrating = (Agent) migrationRequestMessage.getContent();

			MigrationStatusMessage okMsg = new MigrationStatusMessage(agentMigratingAddress, MigrationStatus.OK);
			// remember it by putting in the map of agent to resuscitate
			this.migrationWaitingResuscitate.put(agentMigratingAddress, agentMigrating);
			
			//send a ok message to the sender
			sendMessage(okMsg, sender);
		}
		catch (Exception e) {
			//send an error message to the sender
			MigrationStatusMessage errMsg = new MigrationStatusMessage(agentMigratingAddress, MigrationStatus.ERROR);
			sendMessage(errMsg, sender);
		}
	}

	/**
	 * Process a MigrationStatusMessage
	 * @param migrationStatusMessage the message to process
	 */
	private void processMigrationStatusMessage(MigrationStatusMessage migrationStatusMessage) {
		AgentAddress sender = migrationStatusMessage.getSender();
		AgentAddress agentMigratingAddress = migrationStatusMessage.getAgentMigratingAddress();

		Agent migratingAgent;
		switch (migrationStatusMessage.getStatus()) {
		case OK:
			/* case OK : success of deserialisation in the other kernel, we need to remove
			 the agent from this one. */
			Long isPresentInMigrationWaiting = null;
			synchronized (agentMigratingAddress) {
				isPresentInMigrationWaiting = this.migrationWaitingAgentAddress.remove(agentMigratingAddress);	
			}

			// if we have an agent with this address in wait of a migration
			if(isPresentInMigrationWaiting != null) {
				/* We remove it from the origin kernel  */
				migratingAgent = this.getAgentRepository().get(agentMigratingAddress); 
				if(migratingAgent != null)
					removeAgentFromKernel(migratingAgent);

				//inform the destination address that we deleted the agent from here
				MigrationStatusMessage ackMsg = new MigrationStatusMessage(agentMigratingAddress, MigrationStatus.ACK);
				sendMessage(ackMsg, sender);
			}

			break;
		case ACK:
			Agent toLaunch = this.migrationWaitingResuscitate.get(agentMigratingAddress);

			if(toLaunch != null) {
				this.migrationWaitingResuscitate.remove(agentMigratingAddress);
				toLaunch.setHasMigrated(true);
				launchLightAgent(toLaunch, toLaunch.getName());

				toLaunch.wakeUp();
			}
			else
				System.err.println("Erreur : impossible de reveiller l'agent NULL"); //$NON-NLS-1$

			break;
		case ERROR:
			System.err.println(getAddress() + " ----- Got an ERROR message from " +sender + " ---- about " + agentMigratingAddress ); //$NON-NLS-1$ //$NON-NLS-2$
			isPresentInMigrationWaiting = null;
			synchronized (agentMigratingAddress) {
				isPresentInMigrationWaiting = this.migrationWaitingAgentAddress.remove(agentMigratingAddress);	
			}
			if(isPresentInMigrationWaiting != null) {
				/* We wakeup it from kernel  */
				migratingAgent = this.getAgentRepository().get(agentMigratingAddress); 
				if(migratingAgent != null)
					migratingAgent.wakeUp();
			}
			break;
		default:
			break;
	...
	AgentAddress launchLightAgent(
		...
		if(agent.isMigratingKernel.get()==false){
			if (state==AgentLifeState.ALIVE) {
				if (name!=null)
			...
			agent.isMigratingKernel.set(false);
		}
	...
	private class KernelWrapper implements Kernel {
		...
		@SuppressWarnings("synthetic-access")
		@Override
		public void migrate(Agent agent, AgentAddress destinationKernelAddress) {
			KernelAgent.this.migrate(agent, destinationKernelAddress);			
		}
	}
	...
	private void migrate(Agent agent, AgentAddress destinationKernelAddress) {
		System.out.println(getAddress()+ " ----- Got a migration request from " +agent.getAddress() + "  destination: " + destinationKernelAddress); //$NON-NLS-1$ //$NON-NLS-2$
		agent.sleep(this.TIMEOUT_MIGRATION_WAITING*2);

		agent.isMigratingKernel.set(true);

		Date date = new Date();
		synchronized (this.migrationWaitingAgentAddress) {
			this.migrationWaitingAgentAddress.put(agent.getAddress(), date.getTime() + this.TIMEOUT_MIGRATION_WAITING);
		}

		MigrationRequestMessage requestMigrationMsg = new MigrationRequestMessage(agent); 
		sendMessage(requestMigrationMsg, destinationKernelAddress);



		if(!this.migrationTaskLaunched)
			launchMigrationCheckTask();

	}


	private void launchMigrationCheckTask() {
		...
		Runnable task = new Runnable() {
			...
			public void run() {
				...
				synchronized (KernelAgent.this.migrationWaitingAgentAddress) {
					for (Entry<AgentAddress, Long> item : KernelAgent.this.migrationWaitingAgentAddress.entrySet()) {
						if(item.getValue() > currentTime) {
							KernelAgent.this.migrationWaitingAgentAddress.remove(item.getKey());
						}
					}
				}
			}
		...
		this.context.getScheduledExecutorService().scheduleWithFixedDelay(task, 
				this.TIMEOUT_MIGRATION_WAITING, this.TIMEOUT_MIGRATION_WAITING, TimeUnit.MILLISECONDS);
	}
