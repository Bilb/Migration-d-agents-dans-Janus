...
public final class Kernels {
	...
	/**
	 * Returns a random selected other kernel from the one pass in parameters
	 * @param kernelAddress the address we want to exclude from the result
	 * @return the kernelAddress of an existing different kernel or null if none
	 */
	public static AgentAddress getOtherRandomKernel(AgentAddress kernelAddress) {
		Set<AgentAddress> kernelSet = kernelAgents.keySet();

		for (AgentAddress agentAddress : kernelSet) {
			if(agentAddress.compareTo(kernelAddress) != 0)
				return agentAddress;
		}
